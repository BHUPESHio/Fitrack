  ...
  loginForm?.addEventListener('submit', async (e) => {
    ...
    if (result.success) {
      ...
      if (result.message && result.message.toLowerCase().includes("verify your email")) {
        ...
        if (resendMsg) resendMsg.classList.remove("hidden");
      }
    }
  });
...
// -------------------- Resend Verification --------------------
  window.resendVerification = function (email) {
    ...
    fetch('/resend_verification', {
      ...
      .then(res => res.json())
      .then(data => {
        alert(data.message || data.error || "Verification email sent.");
      })
      .catch(err => {
        console.error(err);
        alert("Error while resending verification.");
      });
  };

}); // End DOMContentLoaded

// ✅ Global OTP verification function
function verifyOtp() {
...
// ✅ Helper: switch to login tab after OTP
  ...
  fetch('/resend_otp', {
    ...
    .then(data => {
      if (data.success) {
        ...
        document.getElementById('otp-message').textContent = "📩 New OTP sent!";
        document.getElementById('otp-message').style.color = "green";
      } else {
        alert(data.message || "Failed to resend OTP.");
        document.getElementById('otp-message').textContent = data.message || "Error resending OTP.";
